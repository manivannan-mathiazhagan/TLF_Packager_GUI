...
def decode_rtf_unicode_escapes(text):
    # Decode RTF Unicode escapes like \u8805?, \u8805 , or \u8805\'3e
    # into their actual characters. Keeps visible symbols such as ≥, ≤, ± intact.
    if not text:
        return ""
    ...
    # Replace \uN followed by either:
    #   - a hex fallback like \'3e
    #   - a single plain fallback character like ? or space
    #   - nothing
    text = re.sub(r"\\u(-?\d+)(?:\\'[0-9a-fA-F]{2}|.)?", repl, text)

    # Remove any remaining standalone hex escapes
    text = re.sub(r"\\'[0-9a-fA-F]{2}", "", text)
    return text

# Strip RTF control words and convert a header group to plain text lines
def rtf_group_to_text(group):
    group = decode_rtf_unicode_escapes(group)
